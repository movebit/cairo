...
impl Backend {
    ...
    async fn refresh_diagnostics(&self) -> LSPResult<()> {
        ...
        let (state, res) = self
            .with_db(|db| {
                let mut state = state;
                // eprintln!("-- refresh_diagnostics 00state.size = {}byte", mem::size_of_val(&state));
                // eprintln!("-- refresh_diagnostics 00state.file_diagnostics.len = {}", state.file_diagnostics.len());
                let mut res = vec![];
                ...
                sys::record_global_memory_usage();
                eprintln!("\n-- refresh_diagnostics --> STEP1: after get all files_set");
                eprintln!("-- refresh_diagnostics current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
                ...
                for file_id in files_set.iter().copied() {
                    ...
                    // sys::record_global_memory_usage();
                    // eprintln!("\n-- refresh_diagnostics --> STEP2_1: after get diagostics of parser");
                    // eprintln!("<< refresh_diagnostics current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());

                    get_diagnostics(db.upcast(), &mut diags, &new_file_diagnostics.semantic);
                    // sys::record_global_memory_usage();
                    // eprintln!("\n-- refresh_diagnostics --> STEP2_2: after get diagostics of semantic");
                    // eprintln!("<< refresh_diagnostics current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());

                    get_diagnostics(db.upcast(), &mut diags, &new_file_diagnostics.lowering);
                    // sys::record_global_memory_usage();
                    // eprintln!("\n-- refresh_diagnostics --> STEP2_3: after get diagostics of lowering");
                    // eprintln!("<< refresh_diagnostics current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
                ...
                sys::record_global_memory_usage();
                eprintln!("\n-- refresh_diagnostics --> STEP2: after get diagostics");
                eprintln!("-- refresh_diagnostics current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
                ...
                // eprintln!("-- refresh_diagnostics 11state.size = {}byte", mem::size_of_val(&state));
                // eprintln!("-- refresh_diagnostics 11state.file_diagnostics.len = {}", state.file_diagnostics.len());
                sys::record_global_memory_usage();
                eprintln!("\n-- refresh_diagnostics --> STEP3: after clearing old diagnostics.");
                eprintln!("-- refresh_diagnostics current_mem = {}Kb, CurTime = {}\n", sys::get_global_memory_usage(), printCurTimeStr());
    ...
    pub async fn reload(&self) -> LSPResult<()> {
        sys::record_global_memory_usage();
        eprintln!(">> reload current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
...
impl LanguageServer for Backend {
    ...
    async fn did_change_watched_files(&self, params: DidChangeWatchedFilesParams) {
        sys::record_global_memory_usage();
        eprintln!(">> did_change_watched_files current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
    ...
    async fn did_open(&self, params: DidOpenTextDocumentParams) {
        sys::record_global_memory_usage();
        eprintln!(">> did_open current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
        let mut db = self.db_mut().await;
    ...
    async fn did_change(&self, params: DidChangeTextDocumentParams) {
        sys::record_global_memory_usage();
        eprintln!(">> did_change current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
        let text =
    ...
    async fn semantic_tokens_full(
        ...
        sys::record_global_memory_usage();
        eprintln!(">> semantic_tokens_full current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
        self.with_db(|db| {
    ...
    async fn hover(&self, params: HoverParams) -> LSPResult<Option<Hover>> {
        sys::record_global_memory_usage();
        eprintln!(">> hover current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
        self.with_db(|db| {
    ...
    async fn goto_definition(
        ...
        sys::record_global_memory_usage();
        eprintln!(">> goto_definition current_mem = {}Kb, CurTime = {}", sys::get_global_memory_usage(), printCurTimeStr());
